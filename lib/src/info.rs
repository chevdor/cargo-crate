...
impl Info {
	...
	pub fn show(response: Vec<ApiResponse>, opts: &InfoOpts) {
		if opts.json {
			...
			Info::show_txt(&response);
		}
	}

	/// Print a formatted output to the console.
	pub fn show_txt(response: &[ApiResponse]) {
		response.iter().for_each(|r| {
			println!("🦀 {}", r.krate.crate_data.name,);

			if let Some(h) = r.krate.crate_data.homepage.as_ref() {
				// println!("{}", h.as_ref().map(String::as_str).unwrap_or(&String::from("")));
				println!("url: {}", h);
			}

			match r.owners.len() {
				1 => println!("owner: {}", r.owners.first().unwrap().name.as_ref().unwrap()),
				x if x > 1 => {
					print!("owners: ");
